...
유저 리소스는 이벤트를 만들거나 조회할 때 사용한다.

[[resources-users-list]]
=== 유저 목록 조회

`GET` 요청을 사용하여 서비스의 모든 유저를 조회할 수 있다.

operation::get-users[snippets='response-fields,curl-request,http-response,links']
...
operation::get-user[snippets='request-fields,curl-request,http-response,links']

[[resources-users-update]]
=== 유저 수정

`PUT` 요청을 사용해서 기존 유저를 수정할 수 있다.

operation::user-event[snippets='request-fields,curl-request,http-response,links']
